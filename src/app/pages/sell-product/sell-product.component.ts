...
export class SellProductComponent {
  ...
  isVerifyUser = true;
  guarantorData: any;
  constructor(private service: ApiService, private builder: FormBuilder, route: ActivatedRoute, private customerService: CustomerDataService, private toaster: ToastrService,private router:Router) {
    service.viewBrandsAppliance().subscribe({
      next: (data) => { // Specify the type of 'data' as Device[]
  ...
  onBrandSelected(event: Event) {
    ...
    this.models = this.arrayOfObject
      .filter((item:any) => item.brand == this.selectedBrandName) // Filter items with the specified brand
  ...
  onModelSelected(event: Event) {
    ...
    this.service.viewBrandsApplianceModel(obj).subscribe({
      next: data => {
